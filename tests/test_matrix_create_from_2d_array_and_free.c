...
int main() {
    int result = test_matrix_create_from_2d_array_and_free();
    if (result == 0) {
        printf("[PASS] test_matrix_create_from_2d_array_and_free passed!\n");
        return 0;
    } else {
        printf("[FAIL] test_matrix_create_from_2d_array_and_free failed!\n");
        return 1;
    }
}
