...
int main() {
    int result = test_pim_frame_misaligned_matrix_multiplication();
    if (result == 0) {
        printf("[PASS] test_pim_frame_misaligned_matrix_multiplication passed!\n");
        return 0;
    } else {
        printf("[FAIL] test_pim_frame_misaligned_matrix_multiplication failed!\n");
        return 1;
    }
}
