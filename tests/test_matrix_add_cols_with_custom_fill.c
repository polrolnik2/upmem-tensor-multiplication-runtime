...
int main() {
    int result = test_matrix_add_cols_with_custom_fill();
    if (result == 0) {
        printf("[PASS] test_matrix_add_cols_with_custom_fill passed!\n");
        return 0;
    } else {
        printf("[FAIL] test_matrix_add_cols_with_custom_fill failed!\n");
        return 1;
    }
}
