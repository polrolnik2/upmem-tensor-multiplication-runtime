...
int main() {
    int result = test_pim_square_multi_tile_identity();
    if (result == 0) {
        printf("[PASS] test_pim_square_multi_tile_identity passed!\n");
        return 0;
    } else {
        printf("[FAIL] test_pim_square_multi_tile_identity failed!\n");
        return 1;
    }
}
